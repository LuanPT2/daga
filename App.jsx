...
const VENV_BIN_DIR = path.join(VENV_DIR, isWindows ? 'Scripts' : 'bin');
const VENV_PYTHON = path.join(VENV_BIN_DIR, isWindows ? 'python.exe' : 'python3');

if (!fs.existsSync(VENV_PYTHON)) {
  console.error(`[INIT] Python venv not found at: ${VENV_PYTHON}`);
  if (isWindows) {
    console.error(`[INIT] Run: cd ${APP_ROOT} && python -m venv .venv`);
  } else {
...
const venvEnv = {
  ...process.env,
  PATH: `${VENV_BIN_DIR}${isWindows ? ';' : ':'}${process.env.PATH}`,
...
const SOURCE_DIR = APP_ROOT;
const SEARCH_SCRIPT = path.join(APP_ROOT, 'search_video.py');
const EXTRACT_SCRIPT = path.join(APP_ROOT, 'extract_features.py');
